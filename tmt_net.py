"""
TMT-Net v13.0 - АВТОНОМНАЯ ВЕРСИЯ
НИКАКИХ НАСТРОЕК. ТОЛЬКО ЗАКОН ПРИРОДЫ.
"""
...
# Начальные условия (как в природе — просто случайность)
np.random.seed(42)
...
# Единственная переменная — время
time = 0.0

print("="*60)
print("TMT-Net v13.0 - АВТОНОМНАЯ")
print("0 настроек. Только закон t = A.")
print("="*60)
...
for epoch in range(500):
    ...
    # ЗАКОН: скорость = 1 + time/50 (ограничена 4)
    speed = min(4.0, 1.0 + time / 50)
    
    # Loss (только для вывода)
    ...
    # Обновление — ТОЛЬКО через скорость (коэффициент = 1, закон природы)
    w2 -= speed * dw2
    b2 -= speed * db2
    w1 -= speed * dw1
    b1 -= speed * db1
